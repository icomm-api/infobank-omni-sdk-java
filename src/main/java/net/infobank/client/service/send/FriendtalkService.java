...
/**
 * FriendtalkService 클래스는 친구톡 서비스와 관련된 작업을 처리합니다.
 ...
 * @since 2023-07-13
 */
public class FriendtalkService extends AbstractMethod<FriendtalkRequest, FriendtalkResponse> {

    /**
     * 친구톡 서비스의 경로를 지정하는 상수입니다.
     */
    private static final String PATH = "/v1/send/friendtalk";

    /**
     * HttpWrapper 객체를 사용하여 FriendtalkService 객체를 생성합니다.
     *
     * @param httpWrapper HTTP 작업을 처리하는 데 사용되는 HttpWrapper 객체입니다.
     */
    public FriendtalkService(HttpWrapper httpWrapper) {
        super(httpWrapper);
    }

    /**
     * 주어진 친구톡 요청을 바탕으로 HTTP 요청을 생성합니다.
     *
     ...
     */
    @Override
    public RequestBuilder makeRequest(FriendtalkRequest friendtalk) throws UnsupportedEncodingException {
    ...
    /**
     * 주어진 HTTP 응답을 바탕으로 친구톡 응답을 파싱합니다.
     *
     ...
     */
    @Override
    public FriendtalkResponse parseResponse(HttpResponse response) throws IOException {
        int statusCode = response.getStatusLine().getStatusCode();
		if (statusCode == 200) {
			return FriendtalkResponse.fromJson(basicResponseHandler.handleResponse(response));
		}
		else {
